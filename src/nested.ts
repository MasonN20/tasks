...
export function findQuestion(
    ...
    let quest = questions.find((question) => question.id === id);
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (curT: number, question: Question) => curT + question.points,
        0,
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (curT: number, question: Question) =>
            question.published ? curT + question.points : curT,
        0,
    );
}
    ...
    return questions.map((question) => {
        ...
        if (question.id === targetId) {
            if (targetOptionIndex === -1) {
                ...
                opt[targetOptionIndex] = newOption;
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce((nq: Question[], question) => {
        nq.push(question);
        if (question.id === targetId) {
            nq.push(duplicateQuestion(newId, question));
        }
        return nq;
    }, []);
}
